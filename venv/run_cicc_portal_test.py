...
if __name__ == '__main__':
    ...
    filename = './ciccportal/report/html/' + now + 'result.html'
    fp = open(filename, 'wb')
    runner = HTMLTestRunner(stream=fp,
                            title='中金研究portal自动化测试报告',
                            description='环境：windows7  浏览器：firefox')
    discover = unittest.defaultTestLoader.discover('./ciccportal/test_case', pattern='*_sta.py')
    runner.run(discover)
    fp.close()
    file_path =new_report('./ciccportal/report/html/')
